<template>
	<StackLayout class="mb-[20dp]">
		...
		<TextField v-bind="$attrs" class="textfield" :class="hasErrors ? 'error' : ''" :text="value" @textChange="onTextChange" @blur="handleBlur" hint="08061234567" keyboardType="phone" />
...
class XPhoneField extends Vue {
	...
	@Watch('value')
	onValueChange(value) {
		this.field.handleChange(this.intl(value))
	}
	get error() {
	...
	intl(value) {
		...
		try {
			const number = phone.parse(value, "NG")
...
export default toNative(XPhoneField)
</script>
